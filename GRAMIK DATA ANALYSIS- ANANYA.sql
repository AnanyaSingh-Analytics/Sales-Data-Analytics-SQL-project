-- Gramik Business Analytics SQL Project
...
SELECT 
    ...
    SUM(quantity_kg) AS sales,
    SUM(SUM(quantity_kg)) OVER(PARTITION BY product ORDER BY DATE_FORMAT(sale_date, '%Y-%m')) AS cumulative_sales
FROM vle_sales
